...
import { ArrowRight, CheckCircle2, Loader2, MailCheck, RefreshCcw, ShieldCheck } from "lucide-react";
...
export default function ResendVerificationPage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      await resendVerificationEmail({ email: values.email.trim() });
      setSubmitState({
        type: "success",
        message: "A fresh verification email is on the way. Open it to activate your BiasLens account.",
      });
    } catch (error) {
      const message =
        error instanceof ApiError
          ? error.message
          : "We could not resend verification right now. Please try again in a moment.";
          ...
          <Card className="w-full rounded-4xl border-[#E7E7E9] bg-white shadow-[0_30px_80px_rgba(13,12,34,0.08)]">
            <CardContent className="p-6 sm:p-8 lg:p-10">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[#6E6D7A] sm:text-base">
                If your activation link expired or never arrived, we&apos;ll send a new one to your inbox.
              </p>

              <Form noValidate onSubmit={onSubmit} className="mt-8 space-y-5">
                <FormField>
                  <FormLabel htmlFor="resend-email">Work Email</FormLabel>
                  <FormInput
                    id="resend-email"
                    type="email"
                    autoComplete="email"
                  ...
                  {errors.email ? (
                    ...
                    <FormHelper>Use the same email you used during registration.</FormHelper>
                  )}
                </FormField>
            ...
            <div className="relative z-10 flex w-full flex-col">
              <div className="rounded-[1.75rem] border border-white/70 bg-white/80 px-5 py-4 backdrop-blur">
                ...
                <p className="mt-3 text-sm leading-6 text-[#6E6D7A]">
                  BiasLens keeps hiring data protected by requiring email activation before workspace access.
                </p>
              </div>

              <div className="mt-6 space-y-4">
                {[
                  { icon: MailCheck, label: "Fresh activation link issued" },
                  { icon: ShieldCheck, label: "Verification protects report access" },
                  { icon: CheckCircle2, label: "One click away from dashboard access" },
                ].map((item) => (
                  <div
                    key={item.label}
