      ...
      <main className="relative z-10 mx-auto w-full max-w-6xl px-0">
        ...
        <Card className="rounded-[2rem] border-[#E7E7E9] bg-white/70 p-6 shadow-[0_24px_64px_rgba(13,12,34,0.06)] backdrop-blur">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Skeleton className="h-10 w-44 rounded-[1.5rem] bg-[#EFF6FF]" />
              <Skeleton className="h-10 w-40 rounded-[1.5rem] bg-[#F3F7FC]" />
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-3">
            {Array.from({ length: 3 }).map((_, i) => (
              ...
              >
                <Skeleton className="h-3.5 w-32 rounded-full bg-[#6E6D7A]/20" />
                <div className="mt-3 h-10 w-full rounded-[1rem] bg-[#F3F7FC] relative overflow-hidden">
                  <div aria-hidden="true" className="absolute inset-0 opacity-70">
                    <ShimmerSweep
                      className="absolute inset-0"
                      delaySeconds={0.12 + i * 0.06}
                    />
                  </div>
                </div>
              </div>
          ...
          <Card className="rounded-[2rem] border-[#E7E7E9] bg-white/70 p-6 shadow-[0_24px_64px_rgba(13,12,34,0.06)] backdrop-blur">
            ...
            <div className="mt-5 space-y-3">
              {Array.from({ length: 7 }).map((_, i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 rounded-[1.5rem] border border-[#E7E7E9] bg-white/60 px-4 py-3"
                >
                  <Skeleton className="h-10 w-10 rounded-2xl bg-[#EFF6FF]" />
                  <div className="min-w-0 flex-1">
                    <Skeleton className="h-4 w-[18rem] max-w-full rounded-full bg-[#F3F7FC]" />
                    <Skeleton className="mt-2 h-3 w-[26rem] max-w-full rounded-full bg-[#F3F7FC]" />
                  </div>
                  <div className="hidden md:block">
                    <Skeleton className="h-10 w-28 rounded-[1.25rem] bg-[#EFF6FF]" />
                  </div>
                  <div className="w-10">
                    <div className="relative h-10 w-10 overflow-hidden rounded-[1.25rem] bg-[#F3F7FC]">
                      <div aria-hidden="true" className="absolute inset-0 opacity-70">
                        <ShimmerSweep
                          className="absolute inset-0"
                          delaySeconds={0.08 + i * 0.05}
                        />
                      </div>
                    </div>
                  </div>
